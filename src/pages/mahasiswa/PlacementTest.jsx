...
export default function PlacementTest() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        // ✅ GET /placement/attempts/:id
        const j = await get(`/placement/attempts/${aid}`);

        if (cancelled) return;

        const at = j?.attempt ?? null;
        setAttempt(at);

        const title = j?.test?.title || j?.attempt?.test?.title || "Placement Test";
        const tl = Number(j?.test?.time_limit ?? j?.attempt?.test?.time_limit ?? 0);
  ...
  useEffect(() => {
    ...
    const limitMin =
      Number(test?.time_limit) > 0 ? Number(test.time_limit) : DEFAULT_LIMIT_MIN;
  ...
  const choose = async (qid, oid) => {
    ...
    try {
      // ✅ POST /placement/attempts/:id/answer
      await post(`/placement/attempts/${aid}/answer`, {
        question_id: qid,
        option_id: oid,
      });
    } catch (err) {
  ...
  const submit = async (auto = false) => {
    ...
    setSubmitting(true);
    try {
      // ✅ POST /placement/attempts/:id/submit
      const j = await post(`/placement/attempts/${aid}/submit`, {});

      if (auto) alert("Waktu habis. Jawabanmu otomatis terkirim.");

      if (j?.review_url) {
        const url = new URL(j.review_url, window.location.origin);
      ...
      <div className="pt-wrap">
        ...
        <div className="header">
          ...
          <div className="badges">
            <span
              className={`badge ${timeLeft !== null && timeLeft <= 60 ? "warn" : ""}`}
            >
          ...
          {loading ? (
            ...
            <>
                        ...
                        {(q?.options ?? []).map((op) => (
                          <label key={op.id} className="opt">
                            <input
                              type="radio"
                              name={`q-${q.id}`}
                              checked={String(answers[q.id] ?? "") === String(op.id)}
              ...
              <div className="stickybar">
                <div className={`tleft ${timeLeft !== null && timeLeft <= 60 ? "warn" : ""}`}>
                ...
                <div style={{ display: "flex", gap: 8 }}>
                  <button
                    className="btn"
                    onClick={() => {
                      if (confirm("Keluar dari tes? Jawaban yang belum tersimpan bisa hilang.")) {
                        navigate("/student");
                      }
                    }}
                  >
                  ...
                  <button
                    ...
                    onClick={() => {
                      if (confirm("Yakin submit jawaban sekarang?")) submit(false);
                    }}
