...
import { get, post, put, ApiError } from "@/config/api";
...
export default function AssignmentBuilder() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        console.error(e);
        const msg =
          e instanceof ApiError ? e.data?.message || e.message : e?.message;
  ...
  const save = async () => {
    ...
    try {
      ...
      if (isEdit) {
        // UPDATE (PUT) — tidak perlu menambah resource lagi
        await put(`/assignments/${assignmentId}`, {
          title: form.title,
        ...
        // CREATE (POST) — lalu tempel ke week sebagai resource
        ...
        const newId = (aJson.assignment || aJson).id;

        const fd = new FormData();
        fd.append("type", "assignment");
        fd.append("title", form.title);
        fd.append("assignment_id", newId);

        await post(`/weeks/${weekId}/resources`, fd);
